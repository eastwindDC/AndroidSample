...
public class MyItemAnimator extends SimpleItemAnimator {
            ...
            if (movesPending) {
                ...
                adder = new Runnable() {
                    public void run() {
                        ...
                        while(var1.hasNext()) {
                            MyItemAnimator.MoveInfo moveInfo = (MyItemAnimator.MoveInfo)var1.next();
                            MyItemAnimator.this.animateMoveImpl(moveInfo.holder, moveInfo.fromX, moveInfo.fromY, moveInfo.toX, moveInfo.toY);
                        }

                        additions.clear();
                        MyItemAnimator.this.mMovesList.remove(additions);
                    }
                };
                if (removalsPending) {
                    View view = ((MyItemAnimator.MoveInfo)additions.get(0)).holder.itemView;
            ...
            if (changesPending) {
                ...
                adder = new Runnable() {
                    public void run() {
                        ...
                        while(var1.hasNext()) {
                            MyItemAnimator.ChangeInfo change = (MyItemAnimator.ChangeInfo)var1.next();
                            MyItemAnimator.this.animateChangeImpl(change);
                        }

                        additions.clear();
                        MyItemAnimator.this.mChangesList.remove(additions);
                    }
                };
                if (removalsPending) {
                    ViewHolder holder = ((MyItemAnimator.ChangeInfo)additions.get(0)).oldHolder;
            ...
            if (additionsPending) {
                ...
                adder = new Runnable() {
                    public void run() {
                        ...
                        additions.clear();
                        MyItemAnimator.this.mAdditionsList.remove(additions);
                    }
                };
    ...
    void animateAddImpl(final ViewHolder holder) {
        ...
        animation.alpha(1.0F).scaleX(1).scaleY(1).setDuration(this.getAddDuration()).setListener(new AnimatorListenerAdapter() {
            public void onAnimationStart(Animator animator) {
    ...
    private void endChangeAnimation(List<MyItemAnimator.ChangeInfo> infoList, ViewHolder item) {
    ...
    public void endAnimation(ViewHolder item) {
        ...
        for(i = this.mPendingMoves.size() - 1; i >= 0; --i) {
            MyItemAnimator.MoveInfo moveInfo = (MyItemAnimator.MoveInfo)this.mPendingMoves.get(i);
        ...
        for(i = this.mMovesList.size() - 1; i >= 0; --i) {
            ...
            for(int j = moves.size() - 1; j >= 0; --j) {
                MyItemAnimator.MoveInfo moveInfo = (MyItemAnimator.MoveInfo)moves.get(j);
                if (moveInfo.holder == item) {
                    view.setTranslationY(0.0F);
                    view.setTranslationX(0.0F);
                    this.dispatchMoveFinished(item);
    ...
    public void endAnimations() {
        ...
        for(listCount = count - 1; listCount >= 0; --listCount) {
            MyItemAnimator.MoveInfo item = (MyItemAnimator.MoveInfo)this.mPendingMoves.get(listCount);
        ...
        if (this.isRunning()) {
            ...
            for(i = listCount - 1; i >= 0; --i) {
                ...
                for(j = count - 1; j >= 0; --j) {
                    MyItemAnimator.MoveInfo moveInfo = (MyItemAnimator.MoveInfo)changes.get(j);
                    ViewHolder itemTmp = moveInfo.holder;
                    View view = itemTmp.itemView;
                    view.setTranslationY(0.0F);
                    view.setTranslationX(0.0F);
            ...
            for(i = listCount - 1; i >= 0; --i) {
                ...
                for(j = count - 1; j >= 0; --j) {
                    ViewHolder itemTmp = (ViewHolder)changes.get(j);
                    View view = itemTmp.itemView;
                    view.setAlpha(1.0F);
                    this.dispatchAddFinished(itemTmp);
                    changes.remove(j);
                    if (changes.isEmpty()) {
            ...
            for(i = listCount - 1; i >= 0; --i) {
                ...
                for(j = count - 1; j >= 0; --j) {
                    this.endChangeAnimationIfNecessary((MyItemAnimator.ChangeInfo)changes.get(j));
                    if (changes.isEmpty()) {
